...
| `navigation` | Optional client navigation. `openAgent({ agentId, serverId? })` and `openWorkspace({ workspaceId, serverId? })` open targets on `serverId`, or on the selected host when omitted. `openCommitDiff({ workspaceId, sha, serverId? })` opens Paseo's commit diff tab for any commit the checkout can reach; it is itself optional on hosts that predate it. `openBrowser({ url, workspaceId, serverId? })` is available only on Electron; see [links and browsers](#external-links-and-workspace-browsers). |
